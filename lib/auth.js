...
function defaultRedirectUri(provider) {
  if (provider === 'google') return `${SITE_ORIGIN}/api/auth-google-callback`;
  return `${SITE_ORIGIN}/api/auth-discord-callback`;
}

export async function handleGoogleCallback(code, redirectUri = defaultRedirectUri('google')) {
  try {
...
export async function handleDiscordCallback(code, redirectUri = defaultRedirectUri('discord')) {
  try {
